                      ...
                      <div class="row">
                        ...
                        <div class="col-xs-6">
                          ...
                          <div class="col-md-6">
                                  ...
                                  <select class="form-control" id="apprna">
                                    <option value="">--- -- ผู้อนุมัติ -- ---</option>
                                    <?php while($row4 = mysql_fetch_array($rs)){?>
                                      <option value="<?php echo $row4['ApprID']; ?>"><?php echo $row4['ApprName'],' / ',$row4['ApprPosition']; ?></option>
                                    <?php } ?>
                                  </select>
                          </div>
                          <div class="col-md-6">
                                      <select class="form-control" id="stoinot">
                                        <option value="">--- นำเข้าสต็อกหรือไม่นำเข้า ---</option>
                                        <option value="0">นำเข้าสต็อก</option>
                                        <option value="1">ไม่นำเข้าสต็อก</option>
                                      </select>
                          </div>
                        </div>
                        <!-- /.col -->
                        <div class="col-xs-6">
                          <p class="lead"><button type="button" class="btn btn-primary " onclick="javascript:calbtn_pvat();">ราคาสินค้าไม่รวมภาษี</button>
                          <button type="button" class="btn btn-info " onclick="javascript:calbtn_vat();">ราคาสินค้ามีภาษี</button>
                          <button type="button" class="btn btn-default" onclick="javascript:calno_vat();">ราคาสินค้าไม่มีภาษี</button></p>
...
function calno_vat() {
  ...
  var co2 = i;
  co2 -= 1;
  for ( var a2 = 1 ; a2 <= co2 ; a2++) {
    suma2 += parseFloat($("#ptotal"+a2).val());
  }
  var sumto1ff = accounting.toFixed(suma2, 2)
  $("#sumpa").val(suma2);
  $("#sumnvat").val(suma2);
  $("#sumvat").val('0.00');
  $("#sumtotal").val(sumto1ff);
}
...
function savepo() {
  ...
  var apprna = $("#apprna").val();
  var stoinot = $("#stoinot").val();
  var data = "compo=" + compo + "&brahpo=" + brahpo + "&suppo=" + suppo + "&pochnum=" + pochnum +"&detailpo=" + detailpo;
      data += "&sumpa=" + sumpa + "&sumnvat=" + sumnvat + "&sumvat=" + sumvat + "&sumtotal=" + sumtotal + "&apprna=" + apprna + "&stoinot=" + stoinot;
	//alert(data);
	if(compo != '' && brahpo != '' && suppo != '' && pochnum != '' && apprna != '' && stoinot != ''){
	$.ajax({
		type: "POST",
