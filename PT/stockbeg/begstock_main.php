...
function addlistbeg() {
  var al = "<tr id='r2"+ i3 +"'>";
  al +=  "<td id='ln2'>"+ i3 +"</td>";
  <?php $sql_pl='SELECT * FROM product_tb,stock_tb WHERE Stk_ProdID = ProdID and StkTotalNow > 0 ';
  ...
  al += '<option value="<?php echo $rowpl['ProdID'];?>" data-tokens="<?php echo $rowpl['ProdName'];?>"><?php echo $rowpl['ProdName'],' ','(',$rowpl['StkTotalNow'],')';?></option>'
        <?php } ?>
  al += '</select>'
  al += '</td>'
